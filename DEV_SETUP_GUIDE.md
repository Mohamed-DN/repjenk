...
In aziende come M-DN il parco database è misto: **Oracle 19c** (la versione Long Term Support
...
### Opzione B: Oracle 19c XE (Express Edition) ⭐ Fondamentale per compatibilità M-DN

Oracle 19c è la versione che troverai sui database PROD di M-DN. Testare anche su 19c
...
|---|---|---|---|
| `m-dn-src-db-credentials` | Username with password | `test_source` | `TestPass123` |
| `m-dn-tgt-db-credentials` | Username with password | `test_target` | `TestPass123` |
| `m-dn-admin-db-credentials` | Username with password | `sys` | `TuaPassword` |
...
databases:
  LOCAL_SOURCE:
    ...
    credential_id: m-dn-src-db-credentials
  ...
  LOCAL_TARGET:
    ...
    port: 1521
    service_name: FREEPDB1
    credential_id: m-dn-tgt-db-credentials
    data_pump_dir: DATA_PUMP_DIR
    data_pump_path: "C:\\oracle\\datapump"
    schemas_allowed: []
```
...
2. Nella sezione Pipeline:
   ...
   - SCM: **Git**
   - Repository URL: il percorso del tuo repo locale (`C:\DBA\m-dn-oracle-datapump-pipeline`)
   - Branch: `*/master`
3. Salva
...
## ☁️ STEP 8: Usare Oracle Cloud Free Tier con Autonomous DB (Come in M-DN!)

Dato che M-DN usa **Autonomous Database** in produzione, il test più realistico è usare
...
Questo è il test più importante perché replica esattamente il flusso M-DN:
...
|---|---|---|
| `m-dn-dev-atp-creds` | Username with password | `admin` / `WelcomeTest#2026` |
| `m-dn-dev-atp-wallet` | Secret file | Il file `Wallet_TestDevATP.zip` |
...
Ora puoi testare il flusso reale M-DN su Jenkins:
...
### Fase 2: Cloud (Autonomous / PL/SQL path — come M-DN!)
