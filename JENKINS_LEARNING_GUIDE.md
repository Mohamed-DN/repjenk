...
## 4. Analisi del Nostro Progetto M-DN Data Pump
...
```groovy
withCredentials([usernamePassword(credentialsId: 'm-dn-src-db-credentials', ...)]) {
    // codice
}
```
Invece di scrivere `admin/Password123!` nel codice, diciamo a Jenkins di andare a prendere la password chiamata `m-dn-src-db-credentials` dalla sua cassaforte segreta. La password viene resa disponibile solo all'interno di quel blocco, e nei log verrà nascosta con degli asterischi (****).
...
2. **Selezione del Job**: Clicchi sul progetto `m-dn-oracle-datapump-pipeline`.
...
```
Tu clicchi "Build" con OPERATION=EXPORT, SOURCE_DB=PROD_ATP_CORE, SCHEMA_NAME=M_DN_CORE
   │
...
```
OPERATION=REFRESH_ENV, SOURCE_DB=PROD_ATP_CORE, TARGET_DB=DEV_ATP_01, SCHEMA_NAME=M_DN_CORE
   │
...
```
OPERATION=SWAP_AND_DROP, TARGET_DB=PROD_DBCS_ERP, SCHEMA_NAME=M_DN_ERP
   │
   ├─ Prerequisiti: Lo schema M_DN_ERP_NEW deve già esistere (creato da un import precedente)
   ├─ Approve → Doppia conferma manuale (solo PROD)
   ├─ Swap → Rinomina M_DN_ERP → M_DN_ERP_BKP_20260714
   │       → Rinomina M_DN_ERP_NEW → M_DN_ERP
   ├─ Verify → Controlla che gli oggetti siano validi
   ├─ (Opzionale) Drop → Elimina M_DN_ERP_BKP_20260714 se DROP_OLD_AFTER_SWAP=true
