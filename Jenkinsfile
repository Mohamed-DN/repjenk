#!/usr/bin/env groovy
// =============================================================================
// ACME Oracle Data Pump Automation Pipeline
// =============================================================================
// Pipeline:    acme-oracle-datapump-pipeline
// Azienda:     ACME S.p.A. — Direzione ICT / Database Operations
...
// Versione:    2.0.0
// Autore:      DBA Team — ACME ICT
// Data:        2026-07-12
// =============================================================================

@Library('acme-oracle-shared-library') _
...
pipeline {
    ...
    environment {
        ...
        SRC_DB_CREDENTIALS        = credentials('acme-src-db-credentials')
        TGT_DB_CREDENTIALS        = credentials('acme-tgt-db-credentials')
        ADMIN_DB_CREDENTIALS      = credentials('acme-admin-db-credentials')

        // --- Wallet per Autonomous Database ---
        ADB_WALLET_DIR            = credentials('acme-adb-wallet-dir')

        // --- Configurazione notifiche ---
        SMTP_CREDENTIALS          = credentials('acme-smtp-credentials')
        DEFAULT_NOTIFICATION_EMAIL = 'dba-team@acme.com'
    ...
    parameters {
        ...
        string(
            ...
            defaultValue: '',
            trim: true,
            description: 'Regole di masking. Es: HR.USERS.EMAIL:ACME_DATA_MASKING.MASK_EMAIL'
        )
        booleanParam(
        ...
        string(
            ...
            defaultValue: '',
            trim: true,
            description: 'Email per notifiche (default: dba-team@acme.com).'
        )
                ...
                script {
                    ...
                    if (params.SOURCE_DB?.trim()) {
                        ...
                        if (srcDb) {
                            ...
                            env.SRC_CRED_ID        = srcDb.credential_id ?: 'acme-src-db-credentials'
                    ...
                    if (params.TARGET_DB?.trim()) {
                        ...
                        if (tgtDb) {
                            ...
                            env.TGT_CRED_ID        = tgtDb.credential_id ?: 'acme-tgt-db-credentials'
                    ...
                    echo """
╔══════════════════════════════════════════════════════════════════╗
║          ACME Oracle Data Pump — Inizializzazione                ║
╠══════════════════════════════════════════════════════════════════╣
        ...
        stage('Validate Parameters') {
            steps {
                script {
                    ...
                    // Politica di sicurezza ACME: ogni operazione che modifica dati su PROD
...
Questa misura è obbligatoria per la policy di sicurezza ACME."""
                    }
        ...
        stage('Export') {
            ...
            steps {
                script {
                    ...
                    // Politica ACME: export da PROD richiede conferma se non è un backup schedulato
        ...
        stage('Import') {
            ...
            steps {
                script {
                    ...
                    // Politica ACME: import su PROD richiede doppia conferma
    ...
    post {
        always {
            script {
                ...
                echo """
╔══════════════════════════════════════════════════════════════════╗
║          ACME Oracle Data Pump — Riepilogo Finale                ║
╠══════════════════════════════════════════════════════════════════╣
...
╚══════════════════════════════════════════════════════════════════╝
        ...
        success {
            script {
                ...
                def subject = "[ACME DataPump] ✓ ${params.OPERATION} completato — ${params.SCHEMA_NAME}@${params.SOURCE_DB}"
                def body = """
<html>
...
<table style="border-collapse: collapse; width: 100%;">
  ...
  <tr><td style="padding: 8px; border: 1px solid #ddd;"><b>Build</b></td><td style="padding: 8px; border: 1px solid #ddd;"><a href="${env.BUILD_URL}">#${env.BUILD_NUMBER}</a></td></tr>
                ...
                // --- Notifica Teams ---
                withCredentials([string(credentialsId: 'acme-teams-webhook', variable: 'TEAMS_WEBHOOK')]) {
        ...
        failure {
            script {
                ...
                def subject = "[ACME DataPump] ✗ FALLITO — ${params.OPERATION} su ${params.SCHEMA_NAME}@${params.SOURCE_DB}"
                def errorLog = ''
                try {
...
<p><b>Azione richiesta:</b> Verificare il log completo su Jenkins e contattare il DBA Team se necessario.</p>
        ...
        unstable {
            script {
                ...
                emailext(
                    to:       env.EFFECTIVE_EMAIL,
                    subject:  "[ACME DataPump] ⚠ INSTABILE — ${params.OPERATION} su ${params.SCHEMA_NAME} — Discrepanze rilevate",
                    body:     """
<html>
<body style="font-family: Arial, sans-serif;">
        ...
        cleanup {
            script {
                ...
                sh """
                    ...
                    rm -f /tmp/acme_dp_*.sql 2>/dev/null || true
