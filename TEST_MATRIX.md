# 🧪 Matrice di Test Completa — ENI Oracle Data Pump Pipeline
...
| Tipologia ENI | Cosa Usa la Pipeline | Come Simularlo Gratis |
|---|---|---|
...
LOCAL_DBCS:
  ...
  credential_id: eni-local-dbcs-creds
...
LOCAL_DBCS_TARGET:
  ...
  port: 1521
  service_name: FREEPDB1
  credential_id: eni-local-target-creds
  data_pump_dir: DATA_PUMP_DIR
  data_pump_path: "C:\\oracle\\datapump"
  oracle_home: "C:\\oracle\\product\\23ai"
  default_tablespace: USERS
...
CREATE USER eni_test IDENTIFIED BY TestPass123
  DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;  -- su locale
  -- oppure DATA su Autonomous
GRANT CONNECT, RESOURCE, CREATE TABLE, CREATE VIEW, CREATE PROCEDURE TO eni_test;

-- Schema target vuoto
CREATE USER eni_test_target IDENTIFIED BY TestPass123
  DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;
GRANT CONNECT, RESOURCE TO eni_test_target;

-- Schema per swap test
CREATE USER eni_test_new IDENTIFIED BY TestPass123
  DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;
GRANT CONNECT, RESOURCE TO eni_test_new;
```

Popola `eni_test` con dati di test diversificati:

```sql
-- Connettiti come eni_test
...
### GRUPPO 2: Flussi Cross-Type (Il Cuore di ENI)
...
|---|---|---|---|---|---|
| 3.1 | Import con REMAP_SCHEMA | `IMPORT` su DBCS | `REMAP_SCHEMA=eni_test:eni_test_target` | Remap schema base | ⬜ |
| 3.2 | Import con REMAP_TABLESPACE | `IMPORT` su DBCS | `REMAP_TABLESPACE=USERS:SYSAUX` | Cambio tablespace | ⬜ |
| 3.3 | Import con CREATE_NEW_SCHEMA | `IMPORT` su DBCS | `CREATE_NEW_SCHEMA=true` | Crea `eni_test_NEW` | ⬜ |
| 3.4 | Remap su Autonomous | `IMPORT` su ATP | `REMAP_SCHEMA=eni_test:eni_test_target` | PL/SQL `METADATA_REMAP` | ⬜ |
...
| # | Test | Operazione | Opzioni | Cosa Testa | Stato |
|---|---|---|---|---|---|
| 7.1 | Export con Data Masking | `EXPORT` | `ENABLE_DATA_MASKING=true`, `MASKING_RULES=eni_test.EMPLOYEES.SSN:ENI_DATA_MASKING.MASK_SSN` | Colonna SSN offuscata | ⬜ |
...
|---|---|---|---|---|---|
| 8.1 | Swap schema DBCS | `SWAP_AND_DROP` su DBCS | Crea `eni_test_NEW` con dati | `DROP_OLD_AFTER_SWAP=false` | ⬜ |
| 8.2 | Swap + Drop DBCS | `SWAP_AND_DROP` su DBCS | Crea `eni_test_NEW` | `DROP_OLD_AFTER_SWAP=true` | ⬜ |
