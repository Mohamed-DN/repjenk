# 🧪 Matrice di Test Completa — M-DN Oracle Data Pump Pipeline
...
| Tipologia M-DN | Cosa Usa la Pipeline | Come Simularlo Gratis |
|---|---|---|
...
LOCAL_DBCS:
  ...
  credential_id: m-dn-local-dbcs-creds
...
CLOUD_ATP:
  ...
  wallet_credential_id: m-dn-dev-atp-wallet
  db_credential_id: m-dn-dev-atp-creds
...
LOCAL_DBCS_TARGET:
  ...
  port: 1521
  service_name: FREEPDB1
  credential_id: m-dn-local-target-creds
  data_pump_dir: DATA_PUMP_DIR
  data_pump_path: "C:\\oracle\\datapump"
  oracle_home: "C:\\oracle\\product\\23ai"
  default_tablespace: USERS
...
CREATE USER m_dn_test IDENTIFIED BY TestPass123
  DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;  -- su locale
  -- oppure DATA su Autonomous
GRANT CONNECT, RESOURCE, CREATE TABLE, CREATE VIEW, CREATE PROCEDURE TO m_dn_test;

-- Schema target vuoto
CREATE USER m_dn_test_target IDENTIFIED BY TestPass123
  DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;
GRANT CONNECT, RESOURCE TO m_dn_test_target;

-- Schema per swap test
CREATE USER m_dn_test_new IDENTIFIED BY TestPass123
  DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;
GRANT CONNECT, RESOURCE TO m_dn_test_new;
```

Popola `m_dn_test` con dati di test diversificati:

```sql
-- Connettiti come m_dn_test
...
### GRUPPO 2: Flussi Cross-Type (Il Cuore di M-DN)
...
|---|---|---|---|---|---|
| 3.1 | Import con REMAP_SCHEMA | `IMPORT` su DBCS | `REMAP_SCHEMA=m_dn_test:m_dn_test_target` | Remap schema base | ⬜ |
| 3.2 | Import con REMAP_TABLESPACE | `IMPORT` su DBCS | `REMAP_TABLESPACE=USERS:SYSAUX` | Cambio tablespace | ⬜ |
| 3.3 | Import con CREATE_NEW_SCHEMA | `IMPORT` su DBCS | `CREATE_NEW_SCHEMA=true` | Crea `m_dn_test_NEW` | ⬜ |
| 3.4 | Remap su Autonomous | `IMPORT` su ATP | `REMAP_SCHEMA=m_dn_test:m_dn_test_target` | PL/SQL `METADATA_REMAP` | ⬜ |
...
| # | Test | Operazione | Opzioni | Cosa Testa | Stato |
|---|---|---|---|---|---|
| 7.1 | Export con Data Masking | `EXPORT` | `ENABLE_DATA_MASKING=true`, `MASKING_RULES=m_dn_test.EMPLOYEES.SSN:M_DN_DATA_MASKING.MASK_SSN` | Colonna SSN offuscata | ⬜ |
...
|---|---|---|---|---|---|
| 8.1 | Swap schema DBCS | `SWAP_AND_DROP` su DBCS | Crea `m_dn_test_NEW` con dati | `DROP_OLD_AFTER_SWAP=false` | ⬜ |
| 8.2 | Swap + Drop DBCS | `SWAP_AND_DROP` su DBCS | Crea `m_dn_test_NEW` | `DROP_OLD_AFTER_SWAP=true` | ⬜ |
